...
exports.getOrCreateRoadSegmentDistanceKm = getOrCreateRoadSegmentDistanceKm;
...
async function getOrCreateRoadSegmentDistanceKm(fromLocationId, toLocationId) {
    ...
    const { data: segment, error: segmentError } = await upabaseClient_1.supabase
        .from("road_segments")
    ...
    const { data: locations, error: locError } = await upabaseClient_1.supabase
        .from("locations")
        .select("id, lat, lng")
    ...
    if (!fromLoc || !toLoc) {
        const missing = [];
        if (!fromLoc)
            missing.push(`origem (${fromLocationId})`);
        if (!toLoc)
            missing.push(`destino (${toLocationId})`);
    ...
    try {
        ...
        const { error: insertError } = await upabaseClient_1.supabase.from("road_segments").insert({
